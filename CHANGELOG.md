...
- Cross-check and contradiction-detection trigger-chain failures are no longer
  silent: the ledger and report now carry `crosscheck_status` and
  `conflict_detection_status` (`ok` / `skipped: <reason>` /
  `failed: <type>: <message>`), so "0 fires" is never indistinguishable from
  "didn't run" (benchmark A4 re-judge depends on this distinction).
